...
export const dadosCarros: VehicleDatabase = {
  "BMW": {
    "116i / 118i (Serie 1)": { inicio: 2004, fim: null, vars: "T_PORTAS" },
    "120i / 125i (Serie 1)": { inicio: 2004, fim: null, vars: "T_PORTAS" },
    "316i / 318i (Serie 3)": { inicio: 1991, fim: 2015, vars: null },
    "320i (Serie 3)": { inicio: 1998, fim: null, vars: null },
    "325i / 328i (Serie 3)": { inicio: 1991, fim: 2019, vars: null },
    "330i / 335i (Serie 3)": { inicio: 2000, fim: null, vars: null },
    "420i / 428i / 430i (Serie 4)": { inicio: 2013, fim: null, vars: ["Coupe", "Gran Coupe", "Cabrio"] },
    "528i / 530i (Serie 5)": { inicio: 1996, fim: null, vars: null },
    "540i / 550i (Serie 5)": { inicio: 1996, fim: null, vars: null },
    "745Li / 750Li (Serie 7)": { inicio: 1990, fim: null, vars: null },
    ...
    "i3 (Eletrico)": { inicio: 2013, fim: 2022, vars: null },
  ...
  "Mercedes-Benz": {
    ...
    "GLC 250 / 300": { inicio: 2015, fim: null, vars: ["SUV", "Coupe"] },
    ...
    "Sprinter (Van/Furgao)": { inicio: 1997, fim: null, vars: null }
  },
  "Fiat": {
    ...
    "Ducato": { inicio: 1998, fim: null, vars: ["Furgao", "Minibus"] },
  ...
  "Ford": {
    ...
    "Transit": { inicio: 2009, fim: null, vars: ["Furgao", "Minibus"] },
  ...
  "Audi": {
    ...
    "A5 Sportback/Coupe": { inicio: 2007, fim: null, vars: null },
...
export const dadosCaminhoes: VehicleDatabase = {
  "Ford Caminhoes": {
    ...
    "Cargo (Leve/Medio)": { inicio: 1985, fim: 2011, vars: null },
    "Cargo (Novo)": { inicio: 2011, fim: 2019, vars: "T_CAM_FORD" }
  },
  "Volkswagen Caminhoes": {
  ...
  "Volvo Caminhoes": {
    ...
    "FH (Classico/Chinezinho)": { inicio: 1994, fim: 2014, vars: "T_CAM_VOLVO_FH" },
  ...
  "Scania": {
    "Serie 3 (113/143)": { inicio: 1991, fim: 1998, vars: null },
    "Serie 4 (114/124)": { inicio: 1998, fim: 2007, vars: "T_CAM_SCANIA_ANTIGO" },
    "Serie 5 (P/G/R)": { inicio: 2008, fim: 2018, vars: "T_CAM_SCANIA_R" },
    "NTG (Nova Geracao)": { inicio: 2019, fim: null, vars: "T_CAM_SCANIA_NTG" }
  },
  "Iveco": {
    "Daily (Chassi/Furgao)": { inicio: 2000, fim: null, vars: null },
  ...
  "Foton Caminhoes": { "Aumark S": { inicio: 2018, fim: null, vars: null }, "Citytruck": { inicio: 2019, fim: null, vars: null } },
...
// Resolve variation string references to actual arrays
const varMaps: Record<string, string[]> = {
  T_PORTAS,
  T_PORTAS_PAJERO,
  T_CAMBIO,
  T_CABINE,
...
export function resolveVars(vars: string[] | string | null): string[] | string | null {
  if (vars === null) return null;
  if (typeof vars === "string" && varMaps[vars]) return varMaps[vars];
  return vars;
}
...
export function getOpcoesPorAno(modelo: string, ano: number): string[] | null {
  ...
  if (nome.includes("s10") || nome.includes("ranger") || nome.includes("hilux") || nome.includes("frontier") || nome.includes("amarok") || nome.includes("triton")) {
    ...
    if (ano > corte) return ["Cabine Simples", "Cabine Dupla (Manual)", "Cabine Dupla (Automatico)"];
    return ["Cabine Simples", "Cabine Dupla"];
  }

  if (nome.includes("strada")) {
    if (ano >= 2020) return ["Cabine Plus (Simples)", "Cabine Dupla (Manual)", "Cabine Dupla (Automatico)"];
...
export function getVariacaoLabel(vars: string[] | string | null, tipo: string): string {
  const resolved = resolveVars(vars);
  if (resolved === T_PORTAS) return "Quantas Portas?";
  if (resolved === T_CAMBIO) return "Tipo de Cambio";
  if (resolved === T_CABINE || resolved === T_CABINE_SD) return "Tipo de Cabine";
  if (resolved === T_PORTAS_PAJERO) return "Tipo de Carroceria";
